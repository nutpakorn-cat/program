...
def reg(name, ia=[-1, -1], ja=[-1, -1], ka=[-1, -1], la=[-1, -1], ma=[-1, -1]):

    if (ia[0] == -1):
        has_already_added = add_variable_to_data(name)
        if has_already_added:
            return
    elif (ja[0] == -1):
        for i in full_range(ia[0], ia[1]):
            varname = name.replace('$1', ',' + str(i))
            has_already_added = add_variable_to_data(varname)
            if has_already_added:
                continue
    elif (ka[0] == -1):
        print("SET " + name + " " + str(ja))
        for i in full_range(ia[0], ia[1]):
